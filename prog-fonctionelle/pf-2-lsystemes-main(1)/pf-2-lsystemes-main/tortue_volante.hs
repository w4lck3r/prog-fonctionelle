...
interpreteSymbole config ((x:xs), (y:ys)) 'F' = ( (avance config x):xs, (positionTortue (avance config x):y):ys)
interpreteSymbole config ((x:xs), (y:ys)) '+' = ( tourneADroite config x:xs, (y:ys) )
interpreteSymbole config ((x:xs), (y:ys)) '-' = ( tourneAGauche config x:xs, (y:ys) )
interpreteSymbole config ((x:xs), (y:ys)) '[' = ( (x:x:xs), (y:y:ys))
interpreteSymbole config ((x:xs), (y:ys)) ']' = ( xs, ys)
...
-- ### TORTUE VOLANTE ###
-- 12
-- on a substitue l'ancien EtatDessin = (EtatTortue, Path) par un nouveau construit des listes
